...
updated_at: 2026-08-09T21:57:43.388Z
closed_at: 2026-08-09T21:57:43.387Z
close_reason: Every watcher stage and combined path budget is bounded, overload cannot silently lose truth, the worker performs no filesystem I/O or blocking send, cancellation/join is prompt, and lifecycle states are typed and deterministically tested.
---
...
Implemented 2026-08-09. Watch ingress is a bounded sync_channel with nonblocking callback try_send and sticky atomic overflow. The worker is an I/O-free coalescer with validated event, pending-path, output, and combined queued-path bounds. Each output is a private bounded CoalescedIntent; consumer-side next_observation/apply_next performs stat, and stable apply_next verifies exactly once. Pending or output loss collapses to a sticky root WatchOverflow intent. Drop sets cancellation, nonblocking-wakes the worker, drops the backend sender, and joins; no worker send blocks. Atomic status distinguishes timeout, clean stop, and caught panic. Tests fill raw, pending, and output stages; prove deferred verification, max-hold behavior, prompt concurrent drop with a full output queue, disconnect/panic typing, no duplicate stable verification, and eventual root reconciliation. Watch-only suite passes 128 tests without timing sleeps; all-feature pedantic Clippy and rustdoc are clean.
