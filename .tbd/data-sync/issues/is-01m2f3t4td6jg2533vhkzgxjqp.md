...
updated_at: 2026-09-14T22:30:54.146Z
...
2026-09-14 guideline review (PR #56 review 5203155693, finding 1): 6f6ff2a fixed only the serial reconcile path, and its test pinned threads: Some(1). 872096b routes all six list-then-stat sites through scan::listed_child_metadata, which answers Ok(None) for NotFound: serial and parallel reconcile and revalidate remove what the baseline held (a vanished .gitignore stays unseen so its rules go), and the serial and concurrent cold walks and opened discovery omit the entry, as they omit a name never listed. The child-metadata hook is now a process-wide registry keyed by root, so wave workers see it; on macOS a hook-covered directory declines getattrlistbulk. Tests loop threads over [1, 4]; the reconcile tests fail at threads 4 without the wave-worker change.
