...
updated_at: 2026-09-14T22:30:41.411Z
...
DECISION (user, 2026-09-14): rename to journal_capacity_bytes and reject values below one commit's cost, with an error naming the unit.

2026-09-14 guideline review (PR #56 review 5203155693, finding 3), implemented on claude/post-merge-engine-fixes: c0adef2 renames OpenOptions::journal_capacity, Python OpenedOptions.journal_capacity and the native keyword to journal_capacity_bytes, and DEFAULT_JOURNAL_CAPACITY to DEFAULT_JOURNAL_CAPACITY_BYTES (structural; the five opened-root goldens re-recorded per scenario, each diff only the field name). 6624295 adds MIN_JOURNAL_CAPACITY_BYTES (512: the 256-byte frame plus one item at the root and the root as its dirty path, the least retained_cost can charge a non-empty commit) and Error::JournalCapacityTooSmall { requested, minimum }, refused at OpenedIndex::open and mapped to InvalidArgumentError in Python; tested in opened_root_rejects_invalid_scan_policy_and_nondirectories and the wheel smoke test. The Python field docstring is still left to #57.
