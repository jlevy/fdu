...
updated_at: 2026-08-11T19:24:06.003Z
...
Audit 2026-08-11: what shipped is registration only, which is what the spec asked for - watch-stream is in the benchmarks/schema.py job vocabulary and scenario-v1.schema.json. There is no runner: measuring a stream needs a harness that samples over time rather than timing a command to completion, which is a different shape from every existing job. Not a gap against this spec; worth its own bead when watch performance actually needs numbers.
