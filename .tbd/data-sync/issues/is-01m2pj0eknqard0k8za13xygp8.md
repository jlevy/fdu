...
2026-09-20 final release audit: the remaining exact Markdown behavior is a documented operational resource limit, not an answer-correctness defect: eligible Markdown still produces exact results, but source plus parser state can grow with the whole active document per worker and can cause OOM on adversarial inputs. There is no small safe bounded patch under the no-truncation/no-size-skip contract; mmap or spooling alone does not bound pulldown-cmark parser state or resolve forward-reference semantics. Keep open for a separately designed exact streaming parser/state model; do not claim globally bounded content-analysis memory.
