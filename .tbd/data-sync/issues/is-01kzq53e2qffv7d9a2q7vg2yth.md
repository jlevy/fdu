...
updated_at: 2026-09-14T23:26:36.036Z
---
...
Review 2026-09-13: the revised spec places gate/fence/publication in fdu-core, with CLI and Python delegation. The historical tens-of-ms expectation is not an accepted whole-command result. Measure dirty-scope work, full-image load/save, cursor age, and fallback against the current scan baseline; see the disk-usage checkpoint plan and fdu-uwhl.

2026-09-14 (PR #55 review, 4727de0): "the journal feature" above is now the history-replay build feature in the FSEvents plan, renamed so it does not collide with the engine's index journal (crates/fdu-core/src/opened/journal.rs). The checkpoint plan's slice 3 adds one acceptance item here: a persistently denied subtree, recorded as a typed gap, must not stop the replay cursor advancing.
