...
pub struct Counts {
    ...
    pub dir_opens: u64,
    /// Complete successful bulk directory listings, not a full enumeration-syscall census.
    ///
    /// Counted only for listings the macOS bulk backend finished, including the empty
    /// terminator. Abandoned work before the portable fallback is omitted. One successful
    /// directory is typically two calls (one data, one EOF), the same multiplicity the
    /// Linux portable path pays in `getdents64`. The portable `read_dir` path leaves this
    /// at 0 because the standard library hides that split. A later Linux or `openat`
