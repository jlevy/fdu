...
class OpenedOptions:
    ...
    #: ``MIN_JOURNAL_CAPACITY_BYTES`` in Rust, the least that retains one commit; a smaller
    #: positive value raises ``InvalidArgumentError`` at open, and the error names the
    #: minimum. Bytes are estimated rather than measured: each retained commit costs a fixed
    #: allowance, plus, for every change, state transition, and dirty path it holds, a fixed
    #: allowance and the bytes of the path it names. A consumer that falls further behind
    #: than the budget is told so with a ``RESET`` outcome and re-reads state; there is no
    #: unbounded setting.
