...
impl PyIndex {
    ...
    fn report<'py>(
        ...
        terminal_extensions: Option<Vec<String>>,
        ancestor_names: Option<Vec<String>>,
    ...
    fn read<'py>(
        ...
        entries_after: Option<Bound<'py, PyAny>>,
        ...
        let entry_page = if entries {
            ...
            Some(fdu_core::EntryPageRequest {
                ...
                after: entries_after.map(|value| parse_entry_cursor(&value)).transpose()?,
...
fn build_query(
    ...
    // Refused rather than repaired, and refused by the engine's own rule rather than by a
    // second one written here: a suffix missing its dot or carrying an uppercase letter
    // matches nothing at all, so a caller who wrote one would otherwise read an empty page
    // as an answer about their tree.
    for value in terminal_extensions.unwrap_or_default() {
        selection.admit_terminal_extension(value).map_err(to_py_err)?;
    }
    for value in ancestor_names.unwrap_or_default() {
        selection.admit_ancestor_name(value).map_err(to_py_err)?;
    }
