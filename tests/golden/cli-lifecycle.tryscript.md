...
```console
$ fdu --cache-status project
[CACHE_FILE]  10 entries, 797 metadata bytes, 0 content bytes  [SCAN_PATH]
? 0
```
...
{
  "caches": [
    {"path": "[CACHE_FILE]", "bytes": 797, "content_bytes": null, "recognized": true, "root": "[SCAN_PATH]", "entries": 10}
  ]
}
? 0
```
