...
| `fdu-u7vo` — fdu for interactive clients: the metabrowser contract | 21 | Shared reads during a write (a measured drop-in blocker), partitioned (gitignore/hidden) tallies, a customizable roll-up taxonomy with a browsing group level, the embedder watch contract, the session integration shape, and the adoption proof. | [interactive client integration](docs/project/specs/active/plan-2026-08-23-fdu-interactive-client-integration.md) |
...
| [interactive client implementation](docs/project/specs/active/plan-2026-08-23-fdu-interactive-client-implementation.md) | Draft | The file- and function-level map for epic `fdu-u7vo`, plus the testing architecture that makes progressive results, watch behaviour, and concurrency ordinary golden tests: a progress mode sharing the watch repaint loop, determinism by injecting `--threads`/`--order`, scripted watch events for the `InvalidateReason` cases, and counter relations as a cost oracle. |
