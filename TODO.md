...
Fourteen open, ordered by how much is outstanding under each.
...
| `fdu-luwc` — surface parity harness: the golden corpus against the library and Python APIs | 1 | Phase 1 landed: 108 of 126 sessions reach parity through the Python API, and every remaining difference carries a named cause. Phase 2 was replaced and landed as the CLI-on-the-public-API split (`fdu-s74c`), which enforces the same property with a crate boundary instead of a second command line. What remains is `fdu-ds2x`, a tryscript improvement. | [python CLI parity](docs/project/specs/active/plan-2026-08-21-fdu-python-cli-parity.md) |
...
| [python CLI parity](docs/project/specs/active/plan-2026-08-21-fdu-python-cli-parity.md) | Phase 1 landed | Phase 2 replaced by [CLI on the public API](docs/project/specs/done/plan-2026-08-22-fdu-cli-on-the-public-api.md), landed. Two Phase 1 items were deferred to tryscript rather than done: `fdu-nluf` and `fdu-ds2x`. |
