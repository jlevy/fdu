...
| `fdu-luwc` — surface parity harness: the golden corpus against the library and Python APIs | 2 | Phase 1 landed: 108 of 126 sessions reach parity through the Python API, and every remaining difference carries a named cause. Phase 2 is the same shim over the Rust library API, which is the instrument that would have caught the copied definitions Phase 1 found by hand. | [python CLI parity](docs/project/specs/active/plan-2026-08-21-fdu-python-cli-parity.md) |
...
| [python CLI parity](docs/project/specs/active/plan-2026-08-21-fdu-python-cli-parity.md) | Phase 1 landed | **Phase 2** (`fdu-esmm`): the same shim over the public Rust library API, with its own deviation file. Two Phase 1 items were deferred to tryscript rather than done: `fdu-nluf` and `fdu-ds2x`. |
