...
| `fdu-snej` — implement the opened-root inventory engine rewrite | 12 | Phase 1 is complete. Phase 2 has landed ownership, discovery, reads, changes, and bounded multi-path refresh; the no-gap observation path has passed its local behavior suites and is awaiting its clean-runner handoff gate. Session proof is next, followed by Python and the joint MetaBrowser integration. Work remains phase-gated on draft PR #48. | [opened-root inventory](docs/project/specs/active/plan-2026-08-25-fdu-opened-root-inventory-engine.md) |
...
| [opened-root inventory engine rewrite](docs/project/specs/active/plan-2026-08-25-fdu-opened-root-inventory-engine.md) | Active; Phase 2 in progress | Epic `fdu-snej`: Phase 1 is complete. No-gap observation has passed its local behavior suites and is awaiting clean-runner validation; session proof, Python, MetaBrowser adoption, and composed acceptance remain open. |
