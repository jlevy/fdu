...
Three specs in `active/` have shipped.
View vocabulary is listed above while its one bead is open; CLI UX and agent skill, and
composable CLI surface, are listed in the archive instead.
Moving all three to `done/` is `fdu-747k`.
