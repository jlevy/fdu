...
| `fdu-snej` — implement the opened-root inventory engine rewrite | 11 | The native Phase 2 lifecycle and five transparent session goldens are complete and all 19 checks pass at `86f64eb`. The synchronous Python wrapper is next. The later MetaBrowser work is decomposed through an unchanged-contract full-lifecycle spike, joint contract, thin adapter, composed wheel proof, and final acceptance; the spike branch is pinned to PR #74 head `3183888`. Work remains phase-gated on draft PR #48. | [opened-root inventory](docs/project/specs/active/plan-2026-08-25-fdu-opened-root-inventory-engine.md) |
...
| [opened-root inventory engine rewrite](docs/project/specs/active/plan-2026-08-25-fdu-opened-root-inventory-engine.md) | Active; native Phase 2 complete | Epic `fdu-snej`: the native lifecycle, no-gap observation, and five-session proof are complete and green through `86f64eb`. Python, the pinned MetaBrowser full-lifecycle spike, joint adoption, and composed acceptance remain open. |
