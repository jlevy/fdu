...
the user workflow this phase should enable: a pinned baseline, a later refresh, and
repeatable signed directory deltas.
Its reproducible FSEvents probe can run before the representation work; the whole-home
latency claim requires both.
Include flat-image load/save cost and the provisional 24-hour replay-age fallback in the
acceptance runs.
