...
- **Shape:** composed from the existing typed parts rather than replacing them:
  `Request { basis: Basis { scope, content }, query, now }`, where `query` carries the
  selection and views.
...
| --- | --- | --- |
| `Basis { scope: ScanConfig, content: AnalysisSet }`, `Request { basis, query: Query, now: SystemTime }`, `RequestError` | Phase 1 item 3 | Every other item |
| `Delivery { cache, cache_path, workers: Workers, accept_partial, watch: Option<WatchDelivery> }` | Fields in Phase 1 item 3; behavior in Phase 2 item 3 | Harness, sessions, execution plan |
| `SnapshotIdentity { entries: EntryTierIdentity, controls: ControlTierIdentity }`, `ContentTierIdentity { entries, analysis, provenance }`, `serves_snapshot`, `entries_writable`, `content_record_writable` | Phase 1 item 2 (`stored_state.rs`) | Projection, execution plan, cache status |
...
| `.github/workflows/ci.yml` | `test` job (`:61-111`), `parity` job (`:228-274`) | Command-line subset on three platforms after `:99`; the two-surface subset in the parity job with `.venv-parity` |
...
Linux. The full matrix is about 20,000 invocations, budgeted at 30 minutes per platform.
...
| File | Function or type | Change |
| --- | --- | --- |
| `stored_state.rs` (new) | `EntryTierIdentity { engine, scope, type_rules_fingerprint, reducers_fingerprint }`, `ControlTierIdentity::{NotObserved, Observed { limits }}`, `SnapshotIdentity { entries, controls }`, `ContentTierIdentity { entries, analysis, provenance }` with `serves` as equality, `entries_writable(&Index)`, `content_record_writable(&Index, &Path, &FileAnalysis)`, and shared fixed-width codecs | Add |
| `snapshot.rs` | `FORMAT_VERSION` (`:64`); `save` (`:216-281`); `put_scope`/`read_scope` (`:924-969`); `read_controls` limits (`:788-791`); `parse_header_fields` (`:599-611`); `parse_stream` (`:614-708`) | Format 5 with header identities; the save guard becomes `entries_writable`; control limits move into the header and a disagreeing table is refused. `identify_prologue` (`:532-566`) keeps its offsets, so format 4 reads as `OlderFormat` |
...
| `lib.rs` | `load_content` (`:719-724`), cache-only check (`:541-551`), `SaveTargets` (`:636-653`), `cold_scan_save_targets_with` (`:699-717`), `spawn_save` (`:730-777`) | Pass the content identity; keep the cache-only count, which now means complete; drop the joint completeness gate (`:736-737`) for per-tier rules; write the sidecar after a partial scan only when a snapshot of the same entry identity is already stored, so cache cleanup does not treat it as orphaned |
...
| `report_format.rs`, `crates/fdu-py/src/lib.rs`, `crates/fdu-py/python/fdu/_models.py` | `CACHE_SCHEMA` (`:70`), `render_cache_status` (`:1610-1720`); `cache_status_dict` (`:1484-1523`); `CacheStatus` model (`:817-836`) | `fdu.cache/2` with identities, coordinated with the answer model |
...
`crates/fdu-py/src/lib.rs:466`, and `examples/perf_probe.rs:501` and `:550`.
...
- `snapshot.rs`: update `semantic_scan_scope_round_trips` (`:2435`) and
  ...
  add `a_v4_snapshot_is_older_format`.
...
**Commits:**
1. `stored_state.rs` types and codecs with unit tests.
2. Snapshot format 5.
3. Sidecar format 5 and `identify_sidecar`.
4. Equality serve; delete `satisfies` and `contains`; clear two registry classes.
...
```rust
pub struct Basis { pub scope: ScanConfig, pub content: AnalysisSet }
...
| --- | --- | --- |
| Size | Allocated | `SizeMetric` defaults to apparent (`query/query_selection.rs:18-19`); the opened-root binding uses `"apparent"` (`crates/fdu-py/src/opened_binding.rs:120`) |
...
| `execution.rs` | `prepare_report` (`:210`), `prepare_report_with_scan_diagnostics` (`:227`), `prepare_report_internal` (`:235`) | Take `(root, &Request, &Delivery)`; delete the check at `:242-244` |
...
`report_in` at `opened/read.rs:267`; `Session::new` at `crates/fdu/src/cli.rs:798`,
...
**Commits:**
1. The model, grammars, defaults, and `validate` with unit tests.
...
5. `Delivery` fields and `validate_delivery`: the watch refusals and the tree default.
...
(`threads`, `batch_size`, `order`) until Phase 2 moves them into `Delivery.workers`.
...
serves: for a stale answer, the pass that wrote the snapshot, read from its header.
...
start; content freshness is stale under cache-only.
Update `scan.rs:8218` and `:8580` for dropped descendants, review `index.rs:8524-8608`,
and regenerate the opened-root golden.
Envelopes are unchanged, so machine goldens should not change.
...
4. One `scan_started_at`, using item 2’s header field.
...
- Paragraphs are counted only when `collect_logical` is on
  (`content/content_basic_metrics.rs:170-178`), which only `words` enables (`:189`), so
  `lines` and `code` report a false zero, and Markdown paragraphs are overwritten by the
  prose analyzer (`:321-329`).

| File | Function or type | Change |
| --- | --- | --- |
| `content/content_model.rs` | `MetricSlotId` (`:18`, unused) | Replace with `MetricDef { name, owner: AnalysisSet, analyzer: AnalyzerId, doc }` and `METRICS`: `lines` owns `physical_lines`, `blank_lines`, `nonblank_lines`, `raw_words`; `code` owns `code_lines`, `comment_lines`, `code_blank_lines`; `words` owns `logical_words`, `paragraphs`, `visible_words`, `visible_logical_words`, `document_words` |
...
| `content/content_index.rs` | `MetricTally` (`:16-45`), `ContentRollUp` (`:49-94`), `commit`, `prepare` | Tally per unit; delete `by_type` and `by_family`, which only tests read |
...
| `examples/perf_probe.rs` | `attach_content_summary` (`:1399-1438`) | Rebuild the digest per unit and version it as `fdu-content-summary-v2` |
...
**Tests:**
- `emit` unit tests: the scalar policy over the 121-string corpus copied to
  `crates/fdu-core/src/testdata/`; the JSON escape set; sink nesting and errors; a
  `SchemaCheck` adapter checking key order and presence against `Field` tables.
- `report_format.rs`: rewrite the YAML quoting, JSON escaping, JSON Lines,
  ...
  name.
- `scripts/check-yaml.mjs`, reusing the `claude/release-e2e-fixes` changes: awkward and
  non-UTF-8 names, `documents`, strict YAML 1.2 (`strict`, `uniqueKeys`, `intAsBigInt`)
  and YAML 1.1 parsing of every document kind deep-equal to exactly parsed JSON (the
  `JSON.parse` reviver’s `context.source`) and to reassembled JSON Lines, across views
  and analyzer sets, cache status, and a watch stream; no raw C1, U+2028, or U+FFFE.
...
pub fn plan(request: &Request, delivery: &Delivery, route: Route) -> Result<Plan, RequestError>;
impl Plan {
...
| `crates/fdu-py/src/lib.rs` | `refresh` (`:456-489`), `watch` (`:336`), `PyWatch.__next__` (`:1115`), `open` (`:1610`), `scan` (`:1685`), `report_once` (`:1319`) | Build a `Delivery`; `refresh` calls core `refresh`; `__next__` calls `persist_due` |
...
**Call sites:** `OpenConfig` literals (39 in `lib.rs` tests, 10 in `execution.rs`,
...
`opened.rs:2192`, `examples/perf_probe.rs`, `watch_session_integration.rs:23`,
...
| File | Function or type | Change |
| --- | --- | --- |
| `stored_state.rs` | `serves_snapshot(stored, wanted) -> Serves::{Exact, ProjectControlsOff, Refuse}` | Add; takes no policy and no consumer |
...
| `query/query_report.rs` | `forget_ignore_classification` (`:1014-1049`), export at `query.rs:21` | Delete |
...
**Commits:**
1. `serves_snapshot` and `load_serving` with tests.
...
- Is `Basis` the right name for what an index holds, against `Held` or a flat field
  list?
