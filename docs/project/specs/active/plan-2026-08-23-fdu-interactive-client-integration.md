...
| Children listing with per-child roll-ups | `children()` clones every child and each directory child’s whole `by_ext` map | **Gap: scalar paged rows** |
...
- Does fdu’s compiled default registry adopt the raw extension level for its own views,
  ...
  Low-stakes either way, since canonical buckets and type rows do not move.
