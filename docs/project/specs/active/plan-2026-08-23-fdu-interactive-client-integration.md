...
- [x] A page continuation is engine-issued, and a stale one is an error (`fdu-91ru`):
  `EntryCursor` has private fields and no constructor, and crosses a boundary as a
  checksummed token rather than a struct a caller could fill in.
  It carries counts the engine established by walking, so a constructible one is an
  honest-looking claim about an answer nobody computed.
  The token also binds the question it was issued for — root, depth bound, plane, and
  the selection’s result-shaping fields — so resuming one assembly under another query
  is refused rather than answered with the first query’s denominator.
  The page bound and the size metric are deliberately outside that shape: a caller may
  change page size mid-assembly, and the metric chooses which number a report renders
  rather than which entries a page admits.

- [x] The consuming contract’s catalog query as closed native predicates (`fdu-8w5k`):
  `Selection.terminal_extensions` and `Selection.ancestor_names`, reachable from the
  library, the command line and Python, and applied by the engine rather than by a
  filter over returned rows.
  Three of the five clauses already translated — files only, an exclusive size bound one
  below an inclusive `max_size` in apparent bytes, and “hide what git ignores” as the
  promoted rule’s plane — and the other two are not globs: a terminal suffix is the
  *last* one and is case-folded, and an ancestor name is a whole component matched
  case-sensitively, any-of.
  A value that could only ever match nothing is refused where it is written, by a rule
  that lives on `Selection` so every surface refuses the same set; the consuming
  contract validates the identical set in its own constructor.
  Both axes are path-shaped, so the live watcher applies them too, through the one
  funnel `admits` itself ends in rather than a hand-written second list.

  Agreement is checked against the other engine’s answers rather than a second reading
  of its rules: `tests/fixtures/catalog-predicates.json` is generated by importing that
  contract’s own `CatalogQuery` and executing its own matcher over a shared corpus.
  Two values the engines judge differently are recorded rather than elided, each with
