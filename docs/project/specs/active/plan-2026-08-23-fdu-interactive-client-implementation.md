...
**The cursor became a token, because a struct with public counts is a claim anyone can
write.** The value carries a denominator and a delivered count that the engine
established by walking; a caller who could construct one could hand back an
honest-looking claim about an answer nobody computed, and the page built from it would
be internally consistent and wrong.
So `EntryCursor` has private fields and no constructor, and crosses a language boundary
as `encode`/`decode` — hex, with a checksum, because the accident that design invites is
a token round-tripped through a wire format that dropped or reordered a field rather
than an attacker.

**And a continuation belongs to one question.** The request carries a shape: the root,
the depth bound, the plane, and the selection’s own result-shaping fields, mixed
explicitly rather than derived.
Resuming under a different question is `Error::InvalidValue`, not an answer with the
first question’s denominator.
Two fields are deliberately outside the shape — the page `limit`, so a caller may change
...
### `fdu-8w5k` — a catalog query is not a glob, in two separate ways

The consuming contract’s catalog query has five clauses, and three of them translate
into the selection that already exists: files only, an exclusive size bound one below an
inclusive `max_size`, and “hide what git ignores” as the promoted rule’s plane.
The other two do not, and the reason is worth stating because the near-miss is so close.

- **A terminal suffix is case-folded; a glob is not.** `*.md` misses `FAQ.MD`, and a
  case-insensitive glob dialect would answer this one question by changing what every
  other pattern means.
  The rule is also the *last* suffix — `archive.tar.gz` is `.gz` — which no `*`-pattern
  says either.
- **An ancestor name is a whole component, case-sensitively, any-of.** `**/src/**` comes
  close and needs one pattern per name, which turns `include`’s any-of into something a
  caller has to reason about, and it is substring-free only by accident of the
  separator.

So they are closed predicate fields rather than a pattern dialect: `terminal_extensions`
and `ancestor_names` on `Selection`, translated by every surface and applied by the
engine. The alternative — filtering rows after the page — is the mirror index this whole
design exists to avoid: it would page the *unfiltered* answer and hand back short pages
with a denominator describing something else.

**Every rule that could only ever match nothing is refused where it is written.** A
suffix without its dot, a compound tail, an uppercase spelling, an ancestor name with a
separator in it: each is a filter a caller believes will match and that nothing can, so
an empty report reads as a fact about their tree.
The rules live on `Selection` (`admit_terminal_extension`, `admit_ancestor_name`) rather
than in each surface’s argument parser, because a rule stated twice is two rules that
agree until one is edited.
The consuming contract validates the identical set in its own constructor, which was
found by reading it rather than assumed — the two engines refuse the same values.

**Both axes are path-shaped, which is where the bug would have been.** They are
decidable from a relative path and a name, so the live watcher has to apply them, and
its filter for removals — the half of the stream that carries no entry to judge — used
to name `include` and `exclude` by hand.
That list is now `Selection::admits_by_path`, which `admits` itself ends in, so the walk
and the watch cannot disagree.
Three separate axes have been forgotten in a hand-written copy of that list; this is the
change that stops there being one.

The agreement is checked against the other engine’s answers rather than against a second
reading of its rules.
`catalog-predicates.json` is generated by importing the consuming contract’s own
`CatalogQuery` and executing its own matcher over a shared corpus, and records what it
refuses and what it admits — including the two values the two engines judge differently,
