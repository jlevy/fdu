...
- [ ] `fdu --skill` prints a skill that names `fdu`, not a placeholder, and states the
  published version after `Generated by fdu`.
- [ ] `fdu --install-skill` in a scratch repository writes `.agents/skills/fdu/SKILL.md`
  and `.claude/skills/fdu/SKILL.md` at its root, and a second run reports both
  `unchanged`.
