...
/ H124 / H125 / H126 / H127 / H128 / H129 / H130 subject (exp-108 through exp-112,
exp-114, exp-115, exp-117, exp-120, exp-121, exp-123, exp-124, exp-125, exp-126,
exp-128, exp-129); same shape as exp-106, engine digest unchanged (`3fbfed48…`). A
2026-09-19 re-observe drifted to 145,988 entries / 133,654 files (digest `cc517e78…`);
commit a fresh subjects document with the next metabrowser cell (`make perf-subjects`).
...
Next free hypothesis id is **H131**. Do not mint another meaning for H91–H106. Next free
experiment id is **exp-113** (reserved unused; H113 superseded).
After that, **exp-130**.

This stacked session skipped H113 (quiet gates including 45.48%, 53.86%, 31.53%, and
34.97%), accepted H125 (exp-124, restore-count completeness), confirmed H126 (exp-125
leftover; completeness gone), confirmed H127 (exp-126; opened-discovery ~8.8×
...
classify gone; `path_of` 11.85%), hunted H107 (no ignore-is-the-walk subject), recorded
exp-122 (H122 leftover), and recorded exp-123 (H113 leftover then 16% of
...
1. **H130** (`fdu-ajbw`). **Confirmed** (exp-129). Restore classify gone (0 of
   `content_open`). Completeness still 0. Snapshot 43.3%. `path_of` 11.85%. No engine
   patch. Quiet this tick 34.97%. **H129** (`fdu-qjjh`). **Accepted** (exp-128). Restore
   omits classify. Wall −13.11% [−20.22%, −12.67%] on frozen `metabrowser-clone`. Engine
   kept (`6887a864`). HashMap and `path_of` stay.
...
Do not retry H104–H106. `path_of` on the restore walk remains after H129 (exp-129:
11.85% of `content_open`); do not mint a cut for it unless the cell names a skip that
keeps completeness.
...
- Do not compile a leftover H130 cut after exp-129. Classify is gone.
  `path_of` is a slice until a later cell names a skip that keeps completeness.
