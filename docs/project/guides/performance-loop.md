...
| H72 | The transient summary needs no directory or symlink attributes, so `d_type` can skip their `statx` calls entirely. | produced stat calls down by the directory share; warm wall down at least 3% on a directory-heavy tree | **Queued, and the shipped default is no longer this tier.** (`fdu-i2f3`). Measured −1.4% alone on a 6.4%-directory tree, below the gate. `.gitignore` default-on makes `fdu --view summary` retain the index; a transient measurement needs `--no-controls` / `--no-gitignore` on both arms (H107). |
