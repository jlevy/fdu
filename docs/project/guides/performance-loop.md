...
| H115 | Sidecar restore still rebuilds roll-ups per file times depth (exp-108: 1,121,963 merges on metabrowser). One bottom-up pass after all restore inserts does the same work in O(files + dirs). That is the remaining named apply/install cut (`fdu-jxhk`), not another per-merge alloc trim. | `content-cache-hit` wall down at least 3% with the interval below zero on deciding-scale metabrowser; content digest identical; intermediate directory roll-ups unchanged | **Queued** (`fdu-jxhk`). Do not start it as an instruction trim. Do not retry H114. |
