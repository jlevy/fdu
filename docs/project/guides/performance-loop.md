...
(currently H121) so no id ever means two things.
...
| --- | --- | --- | --- |
| H107 | On a tree that carries control files, the shipped default (`read_controls` on) does more work than the historical no-controls probe: it reads `.gitignore` and retains ignored-share state. Exclusion can also *remove* work. | `default-tree` with controls on differs from `--no-gitignore` / probe `read_controls=false` by at least 3% wall in either direction on a git-heavy deciding subject; exact tallies differ by the ignored set | **Refuted on wall** (exp-106): +1.64% [−4.00%, +4.37%] on the 146k metabrowser checkout. User CPU +22% and RSS −6.9% show the control walk and the exclusion, and they cancelled on the critical path. Re-run only on a subject whose ignored share can be the walk. |
...
| H113 | After a cache-only sidecar restore, `open_for_report` walks `analysis_candidates` again only to compare `hits` to `len()`. exp-109 sampled that walk at 12.6% of `content_open`. Completeness can use a count already known from restore (or the index file count when the profile is all files) and still refuse an incomplete sidecar. | `content-cache-hit` wall down at least 3% with the interval below zero on deciding-scale metabrowser; content digest identical; incomplete sidecar still refused | **Rejected on wall** (exp-110, uncontrolled): median −7.59% [−10.76%, +2.24%] includes zero. Quiet confirmatory after H115 still needed: 2026-09-19 start gate refused at 46.7% busy (`fdu-rfr6`). exp-113 unused. File-count shortcut not in the engine. Incomplete-sidecar fail-closed test kept. Do not retry uncontrolled. |
...
2026-09-19 overnight queue after H113 quiet lives in
...
| H83 | The content sidecar rebuilds per-record state on load exactly as the metadata snapshot does, so warm content runs are bound by restoring precomputed metrics rather than by analysis. | `--cache only` content load component down several-fold; warm content wall below the profile-independent floor | **Open; scoped to leftover restore/apply after H115.** Sidecar load was about 25 µs/file against 3 µs for a metadata record (`fdu-78q6`). H102 took the file-map order (−31% warm). Path::hash / SipHash was **refuted as H103** (exp-104). H112 (exp-109) split restore: apply 63%, candidates 25%, parse 8.5%. Parse-speed is a dead end. Phase timers kept. H113 (exp-110) rejected the duplicate completeness walk on wall; a quiet confirmatory after H115 still needs a quiet host (2026-09-19 start gate 46.7%). H114 (exp-111) rejected the type-id `String` alloc (−0.56%). H115 (exp-112) accepted one bottom-up pass after restore inserts (−9.69%). Engine kept. H116 (exp-114) rejected the candidate-install HashMap cut on wall (+8.70%; user CPU −15.65%). Engine reverted. Do not retry uncontrolled. First-pass still uses per-file `merge_ancestors` (H118). |
