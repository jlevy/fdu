...
| H118 | First-pass `analyze_index` still `commit`s with `merge_ancestors` per file. H115 deleted that shape on restore only. The same insert-then-`rebuild_rollups` after the receive loop is the leftover apply cut on `content-basic`. | `content-basic` component down at least 3% with the interval below zero on deciding-scale metabrowser; content digest identical | **Open** (`fdu-kyts`). Refute if file I/O hides the ancestor walk. |
