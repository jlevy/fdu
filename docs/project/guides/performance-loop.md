...
| H83 | The content sidecar rebuilds per-record state on load exactly as the metadata snapshot does, so warm content runs are bound by restoring precomputed metrics rather than by analysis. | `--cache only` content load component down several-fold; warm content wall below the profile-independent floor | **Open; wait for H121.** Sidecar load was about 25 µs/file against 3 µs for a metadata record (`fdu-78q6`). H102 took the file-map order (−31% warm). Path::hash / SipHash was **refuted as H103** (exp-104). H112 (exp-109) split restore: apply 63%, candidates 25%, parse 8.5%. Parse-speed is a dead end. Phase timers kept. H113 (exp-110) rejected the duplicate completeness walk on wall; a quiet confirmatory still needs a quiet host. H114 (exp-111) rejected the type-id `String` alloc (−0.56%). H115 (exp-112) accepted one bottom-up pass after restore inserts (−9.69%). Engine kept. H116 (exp-114) rejected the candidate-install HashMap cut on wall (+8.70%; user CPU −15.65%). Engine reverted. H120 (exp-117) accepted streaming parse-into-apply (−10.13% RSS). Do not start another apply/install cut until H121 re-profiles the mix. First-pass apply shape was rejected as H118. |
