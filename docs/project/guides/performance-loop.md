...
| H87 | `spawn_save` deep-clones the entire index synchronously before its thread spawns, so the clone sits on the render path of every cache-writing run — the first-run default shape, not only the changed-warm path. Measured: `--cache off` 860 ms vs `--cache refresh` 1,341 ms on the 450k Linux subject. Sharing the index with the writer (Arc, or serialize-before-return) removes it. | cache-writing end-to-end wall down at least 10% on the 450k subject; `--cache off` unchanged | **Queued** (`fdu-niuz`, priority raised by the 2026-08-15 evidence). |
| H88 | Snapshot CRC-32C is computed one byte at a time through a lookup table over a ~28.7 MB image on both save and load; slicing-by-8 needs no unsafe and hardware CRC32C (SSE 4.2 / ARMv8) is a follow-up behind runtime detection. | `warm-snapshot-load` and `cold-snapshot-save` component down at least 3% | **Confirmed on the pre-registered signal** (exp-057): save component −12.20% [−36.02%, −0.10%], load component −2.02% [−7.00%, −0.34%], both walls improved with intervals below zero; accepted for ~55 dependency-free lines under the exp-009 exception. Hardware CRC32C remains available if the digest’s share grows again. |
| H89 | `derive_ext` allocates a `Vec` per file, validates it with `String::from_utf8`, and `intern_ext` resolves it by string comparison — per file, for an id that directory-listing extension runs make nearly constant. A one-slot `(bytes, ExtId)` memo beside the parent memo removes the allocation, the UTF-8 pass, and most lookups. | cold-scan-index `user_cpu_ns` down and wall down at least 3%, alone or folded into H86 | **Refuted** (exp-056): wall +1.59% [−1.62%, +6.08%] and user CPU regressed +1.42% [+0.39%, +3.98%] — the memo’s own extraction and compare cost what derive-and-intern saved, the H51/H62 shape on a new site. The headroom accrues to H86, where arena entries intern from batch context. |
| H90 | `apply_baseline` clones each batch’s effective ops — `PathBuf`s included — into the journal, then `establish_baseline` clears it: one cloned-and-freed path per entry per cold scan. exp-003 refuted the invasive twin-loop form on macOS at 60k before the consumer was the bottleneck; the residue is a one-flag change. | cold-scan-index `minor_faults` and `user_cpu_ns` down; wall likely under the bar alone, expected to ride with H86 | **Confirmed beyond its own prediction** (exp-058): cold wall −5.06% [−6.03%, −3.54%] on the confirming re-run (−4.62% first), user CPU −6%; the first run’s RSS and warm alarms did not reproduce, and the reproducing context-switch increase costs no wall or CPU. exp-003’s macOS refutation was of a twin-loop form on a pre-consumer-bottleneck build; a parameter on the shared loop measures differently three campaigns later. |
