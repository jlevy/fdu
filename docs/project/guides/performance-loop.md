...
(currently H112) so no id ever means two things.
...
| H83 | The content sidecar rebuilds per-record state on load exactly as the metadata snapshot does, so warm content runs are bound by restoring precomputed metrics rather than by analysis. | `--cache only` content load component down several-fold; warm content wall below the profile-independent floor | **Open; the cheap increment is dead.** Sidecar load was about 25 µs/file against 3 µs for a metadata record (`fdu-78q6`). H102 took the file-map order (−31% warm). The Path::hash / SipHash increment named next was **refuted as H103** (exp-104): instructions fell and wall did not. exp-108 at deciding scale puts `load_content` at 60.5% of the warm content engine and snapshot parse at 25.6%; `install_controls` is 7.5%, not a quarter. What remains is the structural form (`fdu-jxhk`) and a layout usable without rebuilding per-record state — the same class as H78. Do not spend another round on “fewer instructions” on this tier. |
