...
| H124 | First-pass analyze wall is file I/O (`read` 59%, `__open` 17% on deciding-scale `content-basic`). Fewer opens via a type/size gate (do not open files that cannot contribute to the requested analyzer set) or read-ahead on admitted files cuts that wall at least 3%. Not H118 (apply shape). Not H119 (walk overlap / `openat`). | `content-basic` wall or product `--analyze` wall down at least 3% with the interval below zero on deciding-scale metabrowser; digest identical; worker parallelism retained | **Open.** H118/H119 showed apply and walk-overlap cannot move this job. |
