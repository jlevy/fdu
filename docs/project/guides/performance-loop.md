...
| H109 | On a controls-bearing tree, `install_controls` → `reclassify_controlled_subtrees` is about a quarter of the warm content engine, almost all `Path` component comparison (`fdu-hzyb`). A byte-keyed lookup that preserves gitignore precedence and negation should cut wall only if it changes the memory-access pattern the way H102 did, not if it only drops instructions (exp-104). | `content-cache-hit` wall down at least 3% with the interval below zero on a deciding subject that has control files; content digest and control goldens identical | **Queued.** Correctness-gated. Screen on a deciding subject; a 3k-entry tree overstated this tier about 2× in exp-104. |
...
| H83 | The content sidecar rebuilds per-record state on load exactly as the metadata snapshot does, so warm content runs are bound by restoring precomputed metrics rather than by analysis. | `--cache only` content load component down several-fold; warm content wall below the profile-independent floor | **Open; the cheap increment is dead.** Sidecar load was about 25 µs/file against 3 µs for a metadata record (`fdu-78q6`). H102 took the file-map order (−31% warm). The Path::hash / SipHash increment named next was **refuted as H103** (exp-104): instructions fell and wall did not. What remains is the structural form (`fdu-jxhk`) and a layout usable without rebuilding per-record state — the same class as H78. Do not spend another round on “fewer instructions” on this tier. |
