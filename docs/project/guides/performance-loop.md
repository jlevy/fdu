...
| H116 | After H115, candidate install is the largest remaining named restore stage (exp-109: 25.4% of restore / 20.2% of `load_content`). Cache-only restore can match sidecar records to the live index by path without allocating a `Vec`+`HashMap` of every `AnalysisCandidate` (and the classify-every-file walk that builds it). Not H113, which only skipped the second completeness `len()` walk. | `content-cache-hit` wall down at least 3% with the interval below zero on deciding-scale metabrowser; content digest identical; incomplete sidecar still refused | **Open** (`fdu-kro6`). Overnight first after H113 quiet. Uncontrolled OK if quiet fails. |
