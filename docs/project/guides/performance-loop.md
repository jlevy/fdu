...
| H145 | After the current engine, a Linux `opened-discovery` leftover is or is not still H127’s journal clones / `read_dir`+`fstatat` / live merges, and a userspace stage ≥3% remains or does not. Not a `macos_bulk` port. | Determination: named leftover is or is not a new ≥3% userspace cut | **Queued** on [Linux performance iteration](../specs/active/plan-2026-09-20-linux-performance-iteration.md). |
