...
(currently H116) so no id ever means two things.
...
| H83 | The content sidecar rebuilds per-record state on load exactly as the metadata snapshot does, so warm content runs are bound by restoring precomputed metrics rather than by analysis. | `--cache only` content load component down several-fold; warm content wall below the profile-independent floor | **Open; scoped to apply/install.** Sidecar load was about 25 µs/file against 3 µs for a metadata record (`fdu-78q6`). H102 took the file-map order (−31% warm). Path::hash / SipHash was **refuted as H103** (exp-104). H112 (exp-109) split restore on deciding-scale `content-cache-hit`: apply 63% of restore (sample 54% of `load_content`), candidates 25%, parse 8.5%. Parse-speed is a dead end. Phase timers kept. H113 (exp-110) rejected the duplicate completeness walk on wall; a quiet confirmatory after H115 still needs a quiet host (2026-09-19 start gate 46.7%). H114 (exp-111) rejected the type-id `String` alloc on `ContentRollUp::add` (−0.56% [−17.92%, +4.79%]). H115 (exp-112) accepted the remaining named apply/install cut: one bottom-up pass after restore inserts (−9.69% [−26.02%, −7.13%]). Engine kept. Do not start an instruction trim. `fdu-jxhk` remains the EntryId composite; do not restart that rewrite. |
