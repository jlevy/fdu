...
| H87 | `spawn_save` deep-clones the entire index synchronously before its thread spawns, so the clone sits on the render path of every cache-writing run — the first-run default shape, not only the changed-warm path. Measured: `--cache off` 860 ms vs `--cache refresh` 1,341 ms on the 450k Linux subject. Sharing the index with the writer (Arc, or serialize-before-return) removes it. | cache-writing end-to-end wall down at least 10% on the 450k subject; `--cache off` unchanged | **Confirmed on Linux, at its predicted size** (exp-059): the new `cold-open-save` job measured wall −10.50% [−21.96%, −8.11%], component −17.26%, and peak RSS −35.26% [−38.16%, −33.08%] — the second copy of the index disappearing. `cold-scan-index`, which writes no cache, was the placebo and did not move. macOS validation rides with `fdu-m4r6`. |
| H88 | Snapshot CRC-32C is computed one byte at a time through a lookup table over a ~28.7 MB image on both save and load; slicing-by-8 needs no unsafe and hardware CRC32C (SSE 4.2 / ARMv8) is a follow-up behind runtime detection. | `warm-snapshot-load` and `cold-snapshot-save` component down at least 3% | **Confirmed on the pre-registered signal, on Linux** (exp-057): save component −12.20% [−36.02%, −0.10%], load component −2.02% [−7.00%, −0.34%], both walls improved with intervals below zero; accepted for ~55 dependency-free lines under the exp-009 exception. The digest is bit-identical by construction, but the speed claim is Linux evidence: macOS validation is `fdu-m4r6`, and the hardware-CRC32C follow-up (ARMv8 first, so a macOS agent’s item) is `fdu-6kyn`. |
| H89 | `derive_ext` allocates a `Vec` per file, validates it with `String::from_utf8`, and `intern_ext` resolves it by string comparison — per file, for an id that directory-listing extension runs make nearly constant. A one-slot `(bytes, ExtId)` memo beside the parent memo removes the allocation, the UTF-8 pass, and most lookups. | cold-scan-index `user_cpu_ns` down and wall down at least 3%, alone or folded into H86 | **Refuted on Linux** (exp-056): wall +1.59% [−1.62%, +6.08%] and user CPU regressed +1.42% [+0.39%, +3.98%] — the memo’s own extraction and compare cost what derive-and-intern saved, the H51/H62 shape on a new site. The reasoning is glibc-specific, so the refutation is Linux-scoped; `fdu-m4r6` notes the optional libmalloc re-screen. The headroom accrues to H86, where arena entries intern from batch context. |
| H90 | `apply_baseline` clones each batch’s effective ops — `PathBuf`s included — into the journal, then `establish_baseline` clears it: one cloned-and-freed path per entry per cold scan. exp-003 refuted the invasive twin-loop form on macOS at 60k before the consumer was the bottleneck; the residue is a one-flag change. | cold-scan-index `minor_faults` and `user_cpu_ns` down; wall likely under the bar alone, expected to ride with H86 | **Confirmed beyond its own prediction, on Linux** (exp-058): cold wall −5.06% [−6.03%, −3.54%] on the confirming re-run (−4.62% first), user CPU −6%; the first run’s RSS and warm alarms did not reproduce, and the reproducing context-switch increase costs no wall or CPU. exp-003’s macOS refutation was of a twin-loop form on a pre-consumer-bottleneck build; a parameter on the shared loop measures differently three campaigns later. macOS validation rides with `fdu-m4r6`. |
