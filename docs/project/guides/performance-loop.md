...
(currently H55) so no id ever means two things.
...
| H54 | The macOS bulk reader creates and drops one `Vec<Entry>` per directory: 7,350 allocations at 60k and 88,201 at 720k. Retaining the staging vector in each reader and draining it after a successful complete-directory parse should reuse capacity without weakening atomic fallback. | cold/warm `user_cpu_ns` and faults down; at least one primary wall/component down 3%; RSS neutral | **Ready on macOS**: post-exp-026 profiles attribute 7.76–14.90% to allocation; test at 60k and confirm any scale-only effect at 720k. |
