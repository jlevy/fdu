...
| H148 | Profile-guided optimization of the current fat-LTO / one-codegen-unit release probe cuts Linux `cold-scan-index` and `warm-revalidate` wall at least 3% on reconstructible `linux-v6.12`. Screen only: do not change the release profile unless both jobs clear. Not H93 (that id was reused). Not a `PORTABLE` constant. Not an engine source change. | `cold-scan-index` and `warm-revalidate` wall down at least 3% with both intervals below zero on reconstructible `linux-v6.12`; peak RSS no worse | **Queued** (`fdu-pdne`). First experiment id exp-154. |
