...
next free Darwin id is H147) so no id ever means two things.
...
| H85 | The aggregate tier’s cost is not allocation *volume* but glibc’s cross-thread free path: workers allocate, one consumer frees. Returning drained batch buffers to their producing worker would make each arena allocated and freed on one thread, capturing H74’s win without a dependency or its memory cost. | transient-summary wall down at least 20%, i.e. comparable to mimalloc; peak RSS no worse than today | **Hold until H86’s Linux floor stage settles** (`fdu-h7sw`). H86 did not stall: Darwin landed a composite and Linux failed its floor gates (exp-103). Screen against −20%, not 3%, and only if a current-engine profile still shows the cross-thread free. Unmeasured on macOS. The shipped default also observes `.gitignore`, so the historical transient no-controls number is the wrong control (H107). |
