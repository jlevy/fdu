...
| H119 | First-pass analyze opens every admitted file by absolute path after the metadata walk finishes. Submitting analysis from the walk, or `openat` from a retained parent dirfd, overlaps I/O with enumeration. Do not serialize workers (H79). | `content-basic` wall (or product `--analyze=lines`) down at least 3% with the interval below zero on deciding-scale metabrowser; digest identical; worker parallelism retained | **Open** (`fdu-9g54`). Judge on wall, not the `content-basic` component (that timer excludes the setup scan). Profile before changing. |
