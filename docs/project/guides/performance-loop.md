...
| H113 | After a cache-only sidecar restore, `open_for_report` walks `analysis_candidates` again only to compare `hits` to `len()`. exp-109 sampled that walk at 12.6% of `content_open`. Completeness can use a count already known from restore (or the index file count when the profile is all files) and still refuse an incomplete sidecar. | `content-cache-hit` wall down at least 3% with the interval below zero on deciding-scale metabrowser; content digest identical; incomplete sidecar still refused | **Rejected on wall** (exp-110): median −7.59% [−10.76%, +2.24%] includes zero. Component −13.07% [−14.30%, −7.89%] and user CPU −9.79% show the walk. File-count shortcut reverted. Incomplete-sidecar fail-closed test kept. Do not retry on another uncontrolled cell. |
