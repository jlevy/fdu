...
| H113 | After a cache-only sidecar restore, `open_for_report` walks `analysis_candidates` again only to compare `hits` to `len()`. exp-109 sampled that walk at 12.6% of `content_open`. Completeness can use a count already known from restore (or the index file count when the profile is all files) and still refuse an incomplete sidecar. | `content-cache-hit` wall down at least 3% with the interval below zero on deciding-scale metabrowser; content digest identical; incomplete sidecar still refused | **Open; needs quiet host.** Rejected on wall uncontrolled (exp-110): median −7.59% [−10.76%, +2.24%] includes zero. Quiet confirmatory after H115 still needed (`fdu-rfr6`, exp-113 reserved). 2026-09-19 start gates refused at 46.7% and 69.4% busy; later incomplete quiet cells are not a verdict. File-count shortcut not in the engine. Incomplete-sidecar fail-closed test kept. Do not run uncontrolled. |
