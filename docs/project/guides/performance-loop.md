...
| H113 | After a cache-only sidecar restore, `open_for_report` walks `analysis_candidates` again only to compare `hits` to `len()`. exp-109 sampled that walk at 12.6% of `content_open`. Completeness can use a count already known from restore (or the index file count when the profile is all files) and still refuse an incomplete sidecar. | `content-cache-hit` wall down at least 3% with the interval below zero on deciding-scale metabrowser; content digest identical; incomplete sidecar still refused | **Open; needs quiet host.** Rejected on wall uncontrolled (exp-110): median −7.59% [−10.76%, +2.24%] includes zero. Quiet confirmatory after H115 still needed (`fdu-rfr6`, exp-113 reserved). 2026-09-19 start gate refused at 46.7% busy; later incomplete quiet cells are not a verdict. Morning is the intended cell. File-count shortcut not in the engine. Incomplete-sidecar fail-closed test kept. Do not run uncontrolled. |
...
| H119 | First-pass analyze opens every admitted file by absolute path after the metadata walk finishes. Submitting analysis from the walk, or `openat` from a retained parent dirfd, overlaps I/O with enumeration. Do not serialize workers (H79). | `content-basic` wall (or product `--analyze=lines`) down at least 3% with the interval below zero on deciding-scale metabrowser; digest identical; worker parallelism retained | **Screened; walk overlap cannot clear 3%** (`fdu-9g54`). 12 s `sample` on deciding-scale `content-basic` (76,605 stacks): `read` 59.06%, `__open` 17.44%, `fdu::scan` 0.13%, `getattrlistbulk` 0.48%. Overlapping I/O with the walk cannot move 3% wall. The leftover is `__open` via `openat`; that needs a new `unsafe` block, which the overnight bar forbids. No engine change. Do not retry walk-overlap. |
