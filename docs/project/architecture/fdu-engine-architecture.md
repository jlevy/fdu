...
| Stored-state identity | Metadata snapshot, control state, classification, content sidecar, and which requests each may serve | Tier identities, `serves_snapshot`, and the per-tier write rules (`stored_state.rs`); format-5 snapshot and sidecar headers (`snapshot.rs`, `content_cache.rs`), which cache status reports as `fdu.cache/2`; `snapshot_scope_serves` (`lib.rs`); `ContentIndex::prepare` and `load_content_cache` | Partly. Every tier records its identity, serves by equality, and is written by its own rule, but the snapshot’s one report-only projection is coded in `lib.rs` on one route |
...
tier, analyzer set, type rules, options, and analyzer versions.
