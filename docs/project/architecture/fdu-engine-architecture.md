...
| Stored-state identity | Metadata snapshot, control state, classification, content sidecar, and which requests each may serve | Tier identities and `serves_snapshot` (`stored_state.rs`); snapshot and sidecar headers (`snapshot.rs`, `content_cache.rs`); `snapshot_scope_serves` (`lib.rs`); `ContentIndex::prepare` and `load_content_cache` | Partly. Every tier records its identity and serves by equality, but the snapshot’s one report-only projection is coded in `lib.rs` on one route |
