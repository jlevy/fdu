...
It is kept as written, because it is the record of *why* the design is shaped this way
and which prior art each decision came from.
Where building the scaffold has since answered a question this document leaves open, the
plan records the answer rather than this document being quietly rewritten after the
fact.
...
fdu is organized around three artifacts and the contract that connects them:
...
- **The CLI was under-framed until this revision.** Earlier drafts described it as “for
  ...
  handling) must be scheduled as phase-1 work, not deferred as cosmetics (fdu-oqoy).
- **Bootstrapping from `dua-core` conflicts with Goal 1 if it lingers.** dua-core’s stat
  ...
  benchmark gate against dut and gdu passes (fdu-atqk).
- **“Fastest with full stats” must be benchmarked like-for-like.** The fastest walkers
  ...
  claim would be comparing different jobs (fdu-ywu0, which also carries the revalidation
  cost-curve spike).
...
10. **Maintainer sign-off on proposed Goals 6 and 7** (fdu-odx6): the research proposes
